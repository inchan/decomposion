...
def score_concepts(*, produced: Iterable[str], required: Iterable[str] = (), forbidden: Iterable[str] = (), abstained: Iterable[str] = ()) -> Score:
    ...
    return Score(
        ...
        required_hit=len(required_set & produced_set),
        forbidden_total=len(forbidden_set),
        forbidden_hit=len(forbidden_set & produced_set),
        abstain_total=len(abstained_set),
        abstain_hit=len(abstained_set - produced_set),
    )


def passes_smoke_gate(score: Score, *, min_recall: float = 0.8, max_forbidden_rate: float = 0.0, min_abstention_accuracy: float = 1.0) -> bool:
    return (
        score.recall >= min_recall
        and score.forbidden_rate <= max_forbidden_rate
